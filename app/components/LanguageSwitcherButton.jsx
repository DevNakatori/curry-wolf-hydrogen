...
export default function LanguageSwitcherButton({isActive, toggleMenu ,setIsActive, selectedValue , setSelectedValue}) {
  ...
  return (
    ...
    <motion.div 
      ...
      >
                        <AnimatePresence>
                    {isActive && <Languages setSelectedValue = {setSelectedValue} setIsActive = {setIsActive}  isActive = {isActive} />}
                </AnimatePresence>
      </motion.div>
        <motion.div 
        ...
        >
            <div 
                ...
                onClick={() => {toggleMenu()}}
            >
                <PerspectiveText label={label} className={"lang"} Icon={<MdArrowDropDown size={25}  />} />
            </div>
            {/* <div 
                className="LanguageSwitcherButton_el close"
                onClick={() => {toggleMenu()}}
            >
                <PerspectiveText isActive = {isActive} className="close" label={<IoMdCloseCircleOutline size={25} />} />
            </div> */}
        </motion.div>
    </div>
  )
...
function PerspectiveText({label , Icon ,isActive ,className}) {
    ...
    return (    
        ...
        <div className={`${className} perspectiveText`}>
            <p>{label}<span>{Icon}</span></p>
            <p >{className === "lang" && <span><IoEarthOutline size={25} /></span>}{Icon}</p>
        </div>
    )
}